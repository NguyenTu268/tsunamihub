...
class RM_Review_Message_Pro {
	...
	public function display_admin_message() {
		...
		echo sprintf( __('Greetings! I just noticed that you have created %1$d Menus. That is awesome! Could you please help me out by giving this plugin a 5-star rating on WordPress? This will help us by helping other users discover this plugin. %2$s Awesome ! Could you please help me out by giving this plugin a 5 - star rating on WordPress ? This will help us by helping other users discover this plugin. %s', 'responsive - menu - pro'),
			$this->check_results_amount(),
			'<br/><strong><em>~ RM Team</em></strong><br/><br/>'
		);
		echo '<br/><br/><a target="_blank" href="https://wordpress.org/support/plugin/responsive-menu/reviews/#new-topic-0" class="button-primary" > ' . __( 'Yeah, you deserve it ! ', 'responsive - menu - pro' ) . '<a>';
	...
	public function admin_notice_check() {
		if ( isset( $_GET["rm_review_notice_check"] ) && $_GET["rm_review_notice_check"] == 'remove_message' ) {
			...
			if ( $this->trigger === -1 ) {
				exit;
			} elseif ( $this->trigger === 1 ) {
				$update_trigger = 5;
			} elseif ( $this->trigger === 5 ) {
				$update_trigger = 10;
			} elseif ( $this->trigger === 10 ) {
				$update_trigger = -1;
			}
...
$rm_review_message_pro = new RM_Review_Message_Pro();