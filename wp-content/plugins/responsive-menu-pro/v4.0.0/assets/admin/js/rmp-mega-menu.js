jQuery(document).ready(function(jQuery) {
	...
	var  RMP_Mega_Menu = {
		...
		closeSelector : '.rmp-mega-menu-dialog .rmp-dialog-header button.close',
			...
			if ( ! widget.hasClass('loaded') ) {
				...
				jQuery.ajax({
					...
					success: function( response ) {
						...
						$response.find('.widget-delete').on( 'click', function(e) {
							RMP_Mega_Menu.deleteWidget( widget,e );
						});

						$response.find('.widget-close').on( 'click', function(e) {
		...
		addWidget: function(e) {
			...
			jQuery.ajax({
				...
				success: function( response ) {
					...
					var $response = jQuery(response.data);
					$response.find('.rmp-widget-edit').on( 'click', function(e) {
		...
		addRow : function() {
			var self = this;
			jQuery(self.addNewRowSelector).on( 'click', function(e) {
		...
		addCol : function() {
			...
			jQuery(self.megaMenuContainer).on('click',self.addNewColumnSelector, function() {
				...
				var rowUsedCols = parseInt(row.attr('data-row-used-cols'));
				var rowAvailableCols = parseInt(row.attr('data-row-available-cols'));
				if(rowAvailableCols < 1) {
					alert('There is not enough space on this row to add a new column.');
					return;
				}

				var columnSize = (rowAvailableCols >= 4) ? 4 : rowAvailableCols;
				var item_id = jQuery(this).parents('.rmp-mega-menu-dialog').attr('data-item-id');
		...
		deleteCol: function() {
			...
			jQuery(self.megaMenuContainer).on('click', self.columnDeleteSelector, function(e) {
				...
				var rowUsedCols = parseInt(row.attr('data-row-used-cols'));
				var rowSize = parseInt(row.attr('data-row-size'));
				var rowAvailableCols = parseInt(row.attr('data-row-available-cols'));
				var colSize = parseInt(column.attr('data-col-size'));
		...
		optionPopupWindow : function() {
			...
			jQuery(document).on('mousedown', '.popup-controls-toggle', function(e) {
				...
				if( ( jQuery(window).width() - 320 ) < e.pageX ){
					e.pageX = e.pageX - 290;
				}
		...
		saveMegaMenuOptions : function() {
			...
			jQuery( self.megaColumnWrap ).each( function() {
				...
				var col_text_color   = column.find('#rmp-mega-menu-text-color').val();
				var col_text_color_hover   = column.find('#rmp-mega-menu-text-color-hover').val();
