		...
		jQuery( '#rmp-create-new-menu' ).on( 'click', ( e ) => {
			...
			jQuery.ajax( {
				...
				data: {
					...
					'use_in_desktop': jQuery( '#rmp-menu-display-device-desktop' ).is( ':checked' ) ? 'on' : 'off',
					'use_in_tablet': jQuery( '#rmp-menu-display-device-tablet' ).is( ':checked' ) ? 'on' : 'off',
					'use_in_mobile': jQuery( '#rmp-menu-display-device-mobile' ).is( ':checked' ) ? 'on' : 'off'
				},
				type: 'POST',
				dataType: 'json',
				beforeSend: function() {
