...
class Theme_Manager {
	...
	public function rmp_theme_apply() {
		...
		if ( ! current_user_can( 'administrator' ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not apply themes !', 'responsive-menu-pro' ) ) );
		}
		...
		$mobile_menu = sanitize_text_field( $_POST['mobile_menu'] );
		$is_different_menu = sanitize_text_field( $_POST['is_different_menu'] );
		...
		$theme_option['menu_to_use_in_mobile'] = $mobile_menu;
		$theme_option['different_menu_for_mobile'] = $is_different_menu;

		// Update options.
		if ( ! empty( $theme_option['mobile_options'] ) ) {
			update_post_meta( $menu_id, '_mobile', $theme_option['mobile_options'] );
		} else {
			update_post_meta( $menu_id, '_mobile', [] );
		}

		if ( ! empty( $theme_option['desktop_options'] ) ) {
			update_post_meta( $menu_id, '_desktop', $theme_option['desktop_options'] );
		} else {
			update_post_meta( $menu_id, '_desktop', [] );
		}

		if ( ! empty( $theme_option['tablet_options'] ) ) {
			update_post_meta( $menu_id, '_tablet', $theme_option['tablet_options'] );
		} else {
			update_post_meta( $menu_id, '_tablet', [] );
		}
	...
	public function rmp_theme_delete() {
		...
		if ( ! current_user_can( 'administrator' ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not delete themes !', 'responsive-menu-pro' ) ) );
		}
	...
	public function rmp_upload_theme() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not upload themes !', 'responsive-menu-pro' ) ) );
		}
