...
class Widget_Manager {
    ...
    public function rmp_save_widget() {
        ...
        check_ajax_referer( 'rmp_nonce', 'rmp_widget_save_nonce' );

        if ( ! current_user_can( 'administrator' ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not save widget !', 'responsive-menu-pro' ) ) );
		}
    ...
    public function rmp_edit_widget() {
        ...
        if ( ! current_user_can( 'administrator' ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not edit widget !', 'responsive-menu-pro' ) ) );
		}
    ...
    public function rmp_delete_widget() {
        ...
        if ( ! current_user_can( 'administrator' ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not delete widget !', 'responsive-menu-pro' ) ) );
		}
     ...
     public function widget_edit_form( $widget_id ) {
        ...
        printf(
            '<form method="post">
                ...
                <input type="hidden" name="rmp_widget_save_nonce" value="%3$s" />
                <div class="widget-content">',
            esc_attr( $widget_id ),
            esc_attr( $id_base ),
            wp_create_nonce('rmp_nonce')
        );
     ...
     */
     ...
     public function rmp_add_widget() {
        ...
        if ( ! is_admin() ) {
			wp_send_json_error( array( 'message' => __( 'You can not add widget !', 'responsive-menu-pro' ) ) );
		}
        ...
        if ( $widget ) {
           ...
           wp_send_json_error( sprintf( __( 'Failed to add %1$s to %2$d','responsive-menu-pro'), $widget_base_id, $menu_item_id ) );
        }
    ...
    */
     ...
     */
    public function rmp_available_widgets() {
        global $wp_registered_widget_controls;
        ...
        return apply_filters( 'rmp_available_widgets', $available_widgets );
    }
     ...
     * @return array
     */
    public function rmp_get_widget_setting( $widget_id ) {
        // Get all available widgets site supports.
        $available_widgets = $this->rmp_available_widgets();
            ...
            if ( ! empty( $instances ) ) {
                foreach ( $instances as $instance_id => $instance_data ) {
                    if ( is_numeric($instance_id ) ) {
                        ...
                        if ( $unique_instance_id == $widget_id ) {
