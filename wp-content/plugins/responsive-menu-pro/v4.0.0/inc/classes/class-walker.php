...
class Walker extends \Walker_Nav_Menu {
	...
	public function __construct( $options ) {
		...
		if ( function_exists('twentynineteen_add_dropdown_icons') ) {
			remove_filter( 'walker_nav_menu_start_el', 'twentynineteen_add_dropdown_icons', 10 );
		}

		if ( function_exists('twentynineteen_add_mobile_parent_nav_menu_items') ) {
			remove_filter( 'wp_nav_menu_objects', 'twentynineteen_add_mobile_parent_nav_menu_items', 10 );
		}
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( $this->options['use_slide_effect'] == 'on' ) {
			$output .= sprintf('<div class="rmp-go-back"><span class="rmp-go-back-text"> %s %s </span> </div>',
				$this->get_active_arrow(),
				$this->options['slide_effect_back_to_text']
			);
    	}
	}
	...
	public function prepare_mega_menu( $item_id ) {
        ...
        $extra_class = '';
        if ( $this->options['use_desktop_menu'] == 'on' ) {
            $extra_class .= 'rmp-desktop-menu-container rmp-mega-menu-container-' . $item_id;
		}
                ...
                foreach ( $row['columns'] as $col ) {
                    ...
                    foreach ( $col['menu_items'] as $item ) {
                        if ( ! empty( $item['item_type'] ) && 'widget' === $item['item_type'] ) {
							...
							$mega_menu_output .= sprintf( '<div class="rmp-widget">%s</div>', $widget_manager->show_widget( $item['item_id'] ) );
                        } else {
                            $id = get_post_meta( $item['item_id'], '_menu_item_object_id', true );
							$object = get_post_meta( $item['item_id'], '_menu_item_object', true );
            				$type = get_post_meta( $item['item_id'], '_menu_item_type', true );

							if ( $type == 'custom' ) {
								$url = get_post_meta( $item['item_id'], '_menu_item_url', true );
							...
							$item_icon = $this->get_menu_font_icon( $item['item_id'] ); // Set menu item icon.
                            $mega_menu_output .= sprintf(
                                '<a class="rmp-menu-item-link" href="%s">%s %s</a>',
								esc_url( $url ),
								$item_icon,
                                $item['item_title']
                            );
        ...
        $output = '';
		if ( $this->options['use_slide_effect'] == 'on' ) {
			$output .= sprintf('<div class="rmp-go-back"> %s <span class="rmp-go-back-text"> %s </span> </div>',
				$this->get_active_arrow(),
				$this->options['slide_effect_back_to_text']
			);
		}
	...
	public function get_menu_font_icon( $id ) {
        ...
        if ( ! empty( $this->options['menu_font_icons'] ) ) {
			...
			if ( ! empty( $icons['id'] ) && in_array( $id, $icons['id'] ) ) {
				$key = array_search( $id, $icons['id'] );
				if ( ! empty( $icons['custom_icon'][ $key ] ) ) {
					return $icons['custom_icon'][ $key ];
				}else {
