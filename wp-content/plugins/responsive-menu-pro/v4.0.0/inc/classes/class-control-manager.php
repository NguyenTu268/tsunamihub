...
class Control_Manager {
	...
	public function add_text_input_control( $param ) {
		...
		$aria_depend = '';
		if ( ! empty( $param['aria_depend'] ) ) {
			$aria_depend = 'aria-depend=' . $param['aria_depend'];
		}

		$html = sprintf( '<div class="rmp-input-control-wrapper %s" %s >', $group_classes, $aria_depend );
		...
		if ( ! empty( $param['type'] ) ) {
			...
			$html .= sprintf(
				'<input type="%s" id="%s" name="%s" %s class="%s" value="%s" placeholder="%s">',
				esc_attr( $param['type'] ),
				esc_attr( $param['id'] ),
				esc_attr( $param['name'] ),
	...
	public function rmp_textarea_control_html( $param ) {
		...
		/**
		 * Filters the textarea control attributes before create.
		 *
		 * @version 4.4.0
		 * @param array $param List of attribute.
		 */
		...
		// Check tooltip textarea is added or not.
		$tool_tip = '';
		if ( ! empty( $param['tool_tip'] ) ) {
			$tool_tip = $this->get_tool_tip( $param['tool_tip'] );
		}
		 ...
		 * @param array       $param List of attribute.
		 */
		$html = apply_filters( 'rmp_textarea_control_html', $html, $param );
	...
	public function add_color_control( $param ) {
		...
		$html .= sprintf(
			'<input type="text" data-alpha="true" id="%s" name="%s" %s class="no-updates rmp-color-input %s" value="%s">',
			esc_attr( $param['id'] ),
			esc_attr( $param['name'] ),
			esc_attr( $has_multi_device ),
			esc_attr( $class ),
	...
	/**
	 * This function prepare the gradient color control.
	 *
	 * @version 4.0.0
	 * @param array $param  List of attributes for a input control
	 *
	 * @return HTML
	 */
	public function add_gradient_color_control( $param ) {
		...
		/**
		 * Filters the gradient color input control attribute.
		 *
		 * @version 4.0.0
		 * @param array  $param List of attribute.
		 */
		$param = apply_filters( 'rmp_before_add_gradient_color_control', $param );
		...
		$html = sprintf( '<div id="%s" class="%s" %s><div class="rmp-input-control-wrapper">', $param['id'] . '-container', $group_classes, $aria_depend );
		...
		$html .= '<div class="rmp-row"><div class="rmp-input-control col-6">';

		$html .= sprintf(
			'<input type="text" data-alpha="true" id="%s" name="%s" %s class="no-updates rmp-color-input %s" value="%s">',
			esc_attr( $param['id'] . '-first-color' ),
			esc_attr( str_replace( '%type%', 'first_colour', $param['name'] ) ),
			esc_attr( $has_multi_device ),
			esc_attr( $class . ' ' . $param['id'] ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'first_colour', $param['value'] ) ) )
		);

		$html .= '</div><div class="rmp-input-control col-6">';

		$html .= sprintf(
			'<input type="text" data-alpha="true" id="%s" name="%s" %s class="no-updates rmp-color-input %s" value="%s">',
			esc_attr( $param['id'] . '-second-color' ),
			esc_attr( str_replace( '%type%', 'second_colour', $param['name'] ) ),
			esc_attr( $has_multi_device ),
			esc_attr( $class . ' ' . $param['id'] ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'second_colour', $param['value'] ) ) )
		);

		$html .= '</div></div></div>';

		$html        .= '<div class="rmp-input-control-wrapper"><div class="rmp-row"><div class="col-6">';
		$sel_options  = sprintf(
			'<option value="%s" %s> %s </option>',
			esc_attr( 'linear' ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'type', $param['value'] ) ) == 'linear' ? 'selected' : '' ),
			esc_html( 'Linear' )
		);
		$sel_options .= sprintf(
			'<option value="%s" %s> %s </option>',
			esc_attr( 'radial' ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'type', $param['value'] ) ) == 'radial' ? 'selected' : '' ),
			esc_html( 'Radial' )
		);

		$html .= '<div class="rmp-input-control-label"><span>Gradient type</span></div><div class="rmp-input-control">';
		$html .= sprintf(
			'<select id="%s" name="%s" %s class="%s">%s</select>',
			esc_attr( $param['id'] . '-type' ),
			esc_attr( str_replace( '%type%', 'type', $param['name'] ) ),
			esc_attr( $has_multi_device ),
			esc_attr( $class . ' ' . $param['id'] ),
			$sel_options
		);

		$html .= '</div></div><div class="col-6">';

		$html .= '<div class="rmp-input-control-label"><span>Gradient direction</span></div>';
		$html .= sprintf(
			'<input type="range" min="0" max="360" step="1" data-alpha="true" id="%s" name="%s" %s class="%s" value="%s">',
			esc_attr( $param['id'] . '-direction' ),
			esc_attr( str_replace( '%type%', 'direction', $param['name'] ) ),
			esc_attr( $has_multi_device ),
			esc_attr( $class . ' ' . $param['id'] ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'direction', $param['value'] ) ) )
		);
		...
		$html .= '<div class="rmp-input-control-label"><span>Start position</span></div>';
		$html .= sprintf(
			'<input type="range" min="0" max="100" step="1" data-alpha="true" id="%s" name="%s" %s class="%s" value="%s">',
			esc_attr( $param['id'] . '-start-position' ),
			esc_attr( str_replace( '%type%', 'start_position', $param['name'] ) ),
			esc_attr( $has_multi_device ),
			esc_attr( $class . ' ' . $param['id'] ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'start_position', $param['value'] ) ) )
		);

		$html .= '</div><div class="col-6">';

		$html .= '<div class="rmp-input-control-label"><span>End position</span></div>';
		$html .= sprintf(
			'<input type="range" min="0" max="100" step="1" data-alpha="true" id="%s" name="%s" %s class="%s" value="%s">',
			esc_attr( $param['id'] . '-end-position' ),
			esc_attr( str_replace( '%type%', 'end_position', $param['name'] ) ),
			esc_attr( $has_multi_device ),
			esc_attr( $class . ' ' . $param['id'] ),
			esc_attr( rmp_get_value( $options, str_replace( '%type%', 'end_position', $param['value'] ) ) )
		);
		 ...
		 * @param array       $param List of attribute.
		 */
		$html = apply_filters( 'rmp_gradient_color_control_html', $html, $param );
	...
	public function add_button_control( $param ) {
		...
		$html .= sprintf(
			'<button type="button" id="%s" class="button button-primary button-large %s" >%s</button>',
			esc_attr( $param['id'] ),
			esc_attr( $class ),
	...
	public function add_switcher_control( $param ) {
		...
		$disabled = isset( $param['is_disabled'] ) ? $param['is_disabled'] : '';
		$html    .= sprintf(
			'<input type="checkbox" id="%s" name="%s" %s class="toggle %s" value="on" %s %s>',
			esc_attr( $param['id'] ),
			esc_attr( $param['name'] ),
			esc_attr( $has_multi_device ),
			esc_attr( $class ),
	...
	public function add_shortcut_link( $param ) {
		...
		if ( ! empty( $param['label'] ) && ! empty( $param['target'] ) ) {
			$html = sprintf(
				'<div class="rmp-quick-edit-link rmp-input-control-wrapper" aria-owns="%s" accordion-id="%s" sub-accordion-id="%s" sub-tab-id="%s"><a href="javascript:void(0)">%s <i class="fas fa-share"></i></a></div>',
	...
	public function add_icon_picker_control( $param ) {
		...
		$html .= sprintf(
			'<input type="hidden" placeholder="fa fa-icon" id="%s" name="%s" %s class="rmp-icon-hidden-input %s" value="%s">',
			esc_attr( $param['id'] ),
			esc_attr( $param['name'] ),
			esc_attr( $has_multi_device ),
			esc_attr( $class ),
	...
	public function add_image_control( $param ) {
		...
		$html .= sprintf(
			'<input type="hidden" id="%s" name="%s" %s class="rmp-image-url-input %s" value="%s">',
			esc_attr( $param['id'] ),
			esc_attr( $param['name'] ),
			esc_attr( $has_multi_device ),
			esc_attr( $class ),
	...
	/**
	 * Function to return the sub header.
	 *
	...
	/**
	 * Function to returns the items of menu.
	 *
	 * @version 4.0.0
	 *
	 * @return HTML|string
	 */
	public function get_mega_menu_top_items( $options ) {
