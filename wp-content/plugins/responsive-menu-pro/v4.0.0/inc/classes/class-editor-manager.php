...
class Editor_Manager {
	...
	protected function setup_hooks() {
		...
		add_action('wp_ajax_rmp_update_mega_menu_top_items', array( $this, 'rmp_update_mega_menu_top_items' ) );
		add_action('wp_ajax_rmp_current_menu_theme_loaction', array( $this, 'rmp_current_menu_theme_loaction' ) );
	...
	public function check_device_mode( $is_mobile ) {
		...
		if ( ! empty( $_GET['rmp_preview_mode'] ) ) {
			...
			// To get the current device mode in preview page.
			$device = get_transient( 'rmp_device_mode' );
			$rmp_device_mode = ! empty( $device ) ? $device : 'mobile';
		}
	...
	public function rmp_save_mega_menu_item() {
		...
		if ( ! current_user_can( 'edit_post', $menu_id ) ) {
			wp_send_json_error( array( 'message' => __( 'You can not edit menu !', 'responsive-menu-pro' ) ) );
		}
	...
	public function rmp_save_options() {
		...
		// To update the current device mode in preview page.
		set_transient( 'rmp_device_mode', $active_device );
	...
	public function rmp_save_draft_options() {
		...
		$menu_id = sanitize_text_field(filter_input(INPUT_POST, 'menu_id'));
		if ( empty( $menu_id ) ) {
		...
		$save_for_device = sanitize_text_field(filter_input(INPUT_POST, 'save_for_device'));
		$move_on_device = sanitize_text_field(filter_input(INPUT_POST, 'move_on_device'));

		// To update the current device mode in preview page.
		set_transient( 'rmp_device_mode', $move_on_device );
	...
	/**
	 * This function returns theme location is assigned or not.
	 *
	 * @version 4.0.0
	 *
	 * @return json
	 */
	public function rmp_update_mega_menu_top_items() {
		...
		$contents = $control_manager->get_mega_menu_top_items( $options );
	...
	/**
	 * This function returns top level menu items for mega menu.
	 *
	 * @version 4.0.0
	 *
	 * @return json
	 */
	public function rmp_current_menu_theme_loaction() {
		...
		if ( ! empty(  $global_settings['rmp_wp_footer_hook'] ) ) {
			wp_send_json_success( [ 'theme_location' => 'wp_footer_hook' ] );
		}
        ...
        wp_send_json_success( [ 'theme_location' => $theme_location ] );
