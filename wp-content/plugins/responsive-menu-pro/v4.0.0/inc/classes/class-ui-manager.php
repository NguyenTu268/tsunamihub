...
class UI_Manager {
	...
	$col_edit  = sprintf('<a class="rmp-icon-button rmp-action-col-add-widget" href="javascript:void(0)"><span class="dashicons dashicons-plus "></span></a>');
	$col_edit .= sprintf('<a class="rmp-icon-button rmp-action-col-edit popup-controls-toggle" href="javascript:void(0)"><span class="dashicons dashicons-edit "></span></a>');
	...
	$col_edit .= $this->control_manager->add_color_control( [
		...
		'id'    => 'rmp-mega-menu-text-color',
		'name'  => sprintf('menu[deskktop][%s][text_color]', $item_id ),
		'value' => ! empty( $column_meta['text_color']['color'] ) ? $column_meta['text_color']['color'] : '#000',
		'echo'  => false,
	] );

	$col_edit .= $this->control_manager->add_color_control( [
		'label' => __('Text Color Hover','responsive-menu-pro'),
		'id'    => 'rmp-mega-menu-text-color-hover',
		'name'  => sprintf('menu[deskktop][%s][text_color_hover]', $item_id ),
		'value' => ! empty( $column_meta['text_color_hover']['color'] ) ? $column_meta['text_color_hover']['color'] : '#000',
		'echo'  => false,
	] );
	...
	$col_delete   = sprintf('<a class="rmp-icon-button rmp-action-col-delete" href="javascript:void(0)"><span class="dashicons dashicons-no-alt "></span></a>');
	$col_actions  = sprintf('<div class="rmp-mega-menu-col-actions">%s %s</div>', $col_edit, $col_delete );


	$col_collapse = sprintf('<a class="rmp-icon-button rmp-mega-menu-col-collapse" title="Collapse" href="javascript:void(0)">
							<span class="dashicons dashicons-arrow-left-alt2"></span>
					</a>');

	$col_size = sprintf('<span class="rmp-mega-menu-col-size"> %s/12 </span>', esc_html( $column_size) );

	$col_expand = sprintf('<a class="rmp-icon-button rmp-mega-menu-col-expand" title="Expand" href="javascript:void(0)">
	...
	public function prepare_mega_menu_dialog( $menu_id, $item_id ) {
		...
		foreach ( $menu_items as $item ) {
			...
			} elseif ( $item->menu_item_parent == $item_id ) {
				$child_items[] = [
					'item_id'    => $item->ID,
					'item_title' => $item->title,
					'item_type'  => 'menu item',
				];
				$menu_ids[ $item->ID ] = $item->title;
			}
		}
		...
		if ( ! empty( $rmp_mega_item ) ) {
			...
			if ( ! empty( $menu_ids ) ) {
				...
				foreach ( $rows as $row_key => $row ) {
					foreach ( $row['columns'] as $col_key => $columns ) {
						$new_rows = [];
						foreach ( $columns['menu_items'] as $menu_item ) {
							if ( is_numeric( $menu_item['item_id'] ) ) {
								if ( in_array( $menu_item['item_id'], array_keys( $menu_ids ) ) ) {
									$new_menu_ids[] = $menu_item['item_id'];
									$new_rows[] = [
										'item_id'    => $menu_item['item_id'],
										'item_title' => $menu_ids[ $menu_item['item_id'] ],
						...
						$rows[ $row_key ]['columns'][ $col_key ]['menu_items'] = $new_rows;
					}
				}

				$new_items = [];
				$new_menu_item_ids = array_diff( array_keys( $menu_ids ), $new_menu_ids );
				if ( ! empty( $new_menu_item_ids ) ) {
					$previous_items = $rows[0]['columns'][0]['menu_items'];
					foreach ( $new_menu_item_ids as $id ) {
						$new_items[] = [
							'item_id'    => $id,
							'item_title' => $menu_ids[ $id ],
							'item_type'  => 'menu item',
						];
					}
					$rows[0]['columns'][0]['menu_items'] = array_merge( $previous_items, $new_items );
				}
			}
		}
		...
		<section class="rmp-dialog-overlay rmp-mega-menu-dialog" data-item-id="<?php echo $item_id; ?>">
			...
			<div class="rmp-dialog-header">
				<strong class="title">
					...
					<?php echo $item_name; ?>
					<a href="javascript:void(0)" id="rmp-mega-menu-sync"> <?php echo __('Sync live preview..', 'responsive-menu-pro');?></a>
			...
			<div class="rmp-dialog-contents wp-clearfix">
					<div class="rmp-mega-menu-contents">
						<div id="rmp-mega-menu-grid" class="rmp-mega-menu-grid">
							...
							if ( ! empty( $rows ) ) {
								foreach ( $rows as $row ) {
									echo $this->prepare_mega_menu_row( [
										'item_id' => $item_id,
										'row'     => $row,
									] );
								}
